...
import { FirebaseAdapterType } from "./adapter.types";
import { getRealtimeDBMethods } from "./methods/adapter.realtime";

// TODO - add pre and post validation for firebase

export const firebaseAdapter = (database: Database) => {
  const adapter: FirebaseAdapterType = async <R extends RequestInstance>(request: R, requestId: string) => {
