...
export const getNetworkStats = (
  ...
  return {
    ...
    avgResponseTime:
      (currentStats.avgResponseTime * currentStats.totalRequests + responseTime) / currentStats.totalRequests,
    avgResponseTimeSuccess:
      currentStats.totalRequestsSuccess > 0
        ? (currentStats.avgResponseTimeSuccess * currentStats.totalRequestsSuccess + (isSuccess ? responseTime : 0)) /
          currentStats.totalRequestsSuccess
        : 0,
    avgResponseTimeFailed:
      currentStats.totalRequestsFailed > 0
        ? (currentStats.avgResponseTimeFailed * currentStats.totalRequestsFailed + (!isSuccess ? responseTime : 0)) /
          currentStats.totalRequestsFailed
        : 0,
    highestResponseTime: Math.max(currentStats.highestResponseTime, responseTime),
    highestResponseTimeSuccess: isSuccess
      ? Math.max(currentStats.highestResponseTimeSuccess, responseTime)
      : currentStats.highestResponseTimeSuccess,
    highestResponseTimeFailed: !isSuccess
      ? Math.max(currentStats.highestResponseTimeFailed, responseTime)
      : currentStats.highestResponseTimeFailed,
    lowestResponseTime:
      currentStats.lowestResponseTime === 0 ? responseTime : Math.min(currentStats.lowestResponseTime, responseTime),
  ...
  setNetworkStats: async ({ project, data }) => {
    ...
    set((state) =>
      produce(state, (draft) => {
        ...
        const endpointName = getEndpointAndMethod(data.request);
