import { Info } from "lucide-react";

import { Card, CardDescription, CardTitle } from "@/components/ui/card";

export const Tutorial = () => {
  const handleClick = () => {
    window.open("https://hyperfetch.bettertyped.com/docs/integrations/plugin-devtools", "_blank");
  };

  return (
    <Card
      className="relative w-full hover:shadow-md hover:brightness-110 transition-all duration-300 flex items-center justify-center cursor-pointer min-h-[190px]"
      onClick={handleClick}
    >
      <div className="flex flex-col items-center justify-center py-2">
        <div className="h-12 w-12 rounded-full bg-gradient-to-br from-zinc-400/20 to-zinc-700/20 flex items-center justify-center mb-2">
          <Info className="h-6 w-6 text-zinc-300" />
        </div>
        <CardTitle className="opacity-70 mb-2 mt-3">Cannot see your application?</CardTitle>
        <CardDescription className="opacity-70">Learn how to connect it to the DevTools</CardDescription>
      </div>
    </Card>
  );
};
