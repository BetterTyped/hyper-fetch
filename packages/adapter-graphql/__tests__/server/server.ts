...
export const createRequestInterceptor = <T extends RequestInstance, StatusType extends StatusCodesType>(
  ...
  const { endpoint, client } = request;
  const url = getInterceptEndpoint(client.url);
  ...
  if (currentStatus !== 200 && currentStatus in errorResponses) {
    ...
    server.use(createStubMethod(request, url, endpoint, currentStatus, errorResponse, delay) as any);

    return errorResponse;
  }

  const responseData = (fixture || { data: [1, 2, 3] }) as ExtractResponseType<T>;
  server.use(createStubMethod(request, url, endpoint, currentStatus, responseData, delay) as any);
  return responseData;
};
