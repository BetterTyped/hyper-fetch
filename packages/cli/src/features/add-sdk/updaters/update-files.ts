...
import { RegistryItemSchema, registryItemFileSchema } from "features/schema/schema";
...
import { findExistingEnvFile, getNewEnvKeys, isEnvFile, mergeEnvContent, parseEnvContent } from "utils/env-helpers";
...
export async function updateFiles(
  files: RegistryItemSchema["files"],
  config: Config,
  options: {
    overwrite?: boolean;
    force?: boolean;
  ...
  if (envVarsAdded.length && envFile) {
    ...
    if (!options.silent) {
      for (const key of envVarsAdded) {
        logger.info(`  ${highlighter.success("+")} ${key}`);
      }
    }
  }
...
export function toAliasedImport(filePath: string, config: Config, projectInfo: ProjectInfo): string | null {
  ...
  const aliasBase =
    aliasKey === "cwd" ? projectInfo.aliasPrefix : config.aliases[aliasKey as keyof typeof config.aliases];
  if (!aliasBase) {
    return null;
  }
