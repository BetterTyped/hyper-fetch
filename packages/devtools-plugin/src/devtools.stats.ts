...
export class DevtoolsQueueStats {
  ...
  requestsCancelled: DevtoolsElement[] = [];
  requestsRemoved: DevtoolsElement[] = [];
  ...
  addCancelledRequest = ({ requestId, request }: { requestId: string; request: RequestInstance }) => {
    this.requestsCancelled.push({
      requestId,
  ...
  addRemovedRequest = ({ requestId, request }: { requestId: string; request: RequestInstance }) => {
    this.requestsRemoved.push({
      requestId,
      queueKey: request.queueKey,
      cacheKey: request.cacheKey,
      abortKey: request.abortKey,
