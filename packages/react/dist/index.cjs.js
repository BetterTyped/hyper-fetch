...
var useCommandEvents = ({
  ...
  const onRequestCallback = (0, import_react.useRef)(null);
...
var useFetch = (command, {
  ...
  const revalidate = (invalidateKey) => {
    if (invalidateKey && invalidateKey instanceof import_hyper_fetch2.Command) {
      cache.events.revalidate(new RegExp((0, import_hyper_fetch2.getCommandKey)(invalidateKey, true)));
...
var useSubmit = (commandInstance, {
  ...
  const [command, setCommand] = (0, import_react5.useState)(commandInstance);
  ...
  const handleSubmit = (...parameters) => {
    ...
    const triggerRequest = () => (0, import_hyper_fetch4.commandSendRequest)(commandClone, "submit", (requestId, cmd) => {
      addDataListener(cmd);
      addLifecycleListeners(requestId);
    });
  ...
  const revalidate = (invalidateKey) => {
    ...
    if (invalidateKey && invalidateKey instanceof import_hyper_fetch4.Command) {
      cache.events.revalidate(`/${(0, import_hyper_fetch4.getCommandKey)(invalidateKey, true)}/`);
    } else {
      cache.events.revalidate(invalidateKey);
    }
  };
...
var useQueue = (command, options = useQueueDefaultOptions) => {
  ...
  (0, import_react_lifecycle_hooks5.useDidMount)(getInitialState2);
  (0, import_react_lifecycle_hooks5.useDidUpdate)(mountEvents, [stopped, requests, setRequests, setStopped], true);
  return {
    stopped,
    requests,
...
var useAppManager = (builder) => {
  ...
  (0, import_react_lifecycle_hooks6.useDidMount)(mountEvents);
