...
export class RequestEffect<T extends RequestInstance> {
  ...
  onSuccess = (response: ResponseSuccessType<ExtractResponseType<T>, ExtractAdapterType<T>>, request: T) => {
    this.config.onSuccess?.(response, request);
  };
  onError = (response: ResponseErrorType<ExtractErrorType<T>, ExtractAdapterType<T>>, request: T) => {
