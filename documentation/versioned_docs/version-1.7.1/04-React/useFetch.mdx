...
This hook is created to **retrieve data** from the server. It uses the [`Fetch Dispatcher`](/docs/Core/Dispatcher) to
handle requests and the [`Cache`](/docs/Core/Cache) to manage the overall state of the data.

A prepared Command is the minimum requirement for useFetch.

If you intend to `mutate` data stored on the server, we recommend choosing the [`useSubmit`](/docs/React/useSubmit)
hook.
