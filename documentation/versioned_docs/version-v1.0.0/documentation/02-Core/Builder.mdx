...
[Read the API Reference »](/api/Hyper-Fetch/Class/Builder.md)
...
**`Builder`** is a class that allows you to configure the server connection. It initializes the entire library’s
...
It was designed to be used as a singleton, where `Builder` helps us create a global structure for making server requests
...
To send authenticated requests, set up the `onAuth` interceptor. Set up the command with the `auth` option set to
...
Use the `onRequest` builder method if you need to use the pre-request interceptor to modify the command before it’s
sent.
...
- `GlobalErrorType` defines the error type used in all the commands. It should consist of an `Error` type and your
  default `ServerErrorType`. For some command’s individual error types, you can set up a `LocalErrorType` for each
  command.

- `RequestConfigType` is the generic responsible for shaping options passed to the client. Most likely you will change
  it only when you provide your custom client.

---

## Components

### [Cache](/api/Hyper-Fetch/Class/Cache.md)

Handles data storages and persistence. Can be adjusted with options when initializing `Builder`.

---

### [Client](/api/Hyper-Fetch/Function/fetchClient.md)

Handles all requests within `Builder`. Can be replaced with `setClient` method.

---

### [SubmitDispatcher](/api/Hyper-Fetch/Class/Dispatcher.md)

Handles the mutation requests and queueing. Can be adjusted with options when initializing the builder.

---

### [FetchDispatcher](/api/Hyper-Fetch/Class/Dispatcher.md)

Handles the fetching requests and queueing. Can be adjusted with options when initializing the builder.

---

### [AppManager](/api/Hyper-Fetch/Class/AppManager.md)

Handles the app focus and online state. Can be adjusted with options when initializing the builder.

---

### [CommandManager](/api/Hyper-Fetch/Class/CommandManager.md)

Handles additional events and cancellation of requests. Can be adjusted with options when initializing the builder.

---

### [LoggerManager](/api/Hyper-Fetch/Class/LoggerManager.md)
...
(@import HyperFetch BuilderConfig type=returns)
...
```tsx
export const builder = new Builder({ url }).onError(async (res, command) => {
  const status = res[2];
