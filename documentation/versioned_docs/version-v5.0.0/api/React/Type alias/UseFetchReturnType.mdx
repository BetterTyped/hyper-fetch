...
Defined in [hooks/use-fetch/use-fetch.types.ts:87](https://github.com/BetterTyped/hyper-fetch/blob/cc329b91/packages/react/src/hooks/use-fetch/use-fetch.types.ts#L87)
...
```ts
type UseFetchReturnType<T> = UseTrackedStateType<T> & UseTrackedStateActions<T> & UseRequestEventsActionsType<T> & { bounce: { active: boolean; reset: () => void }; refetch: (invalidateKey?: InvalidationKeyType | InvalidationKeyType[]) => void };
```
