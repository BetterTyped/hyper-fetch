

# RealtimeDBQueryParams

<div class="api-docs__separator" data-reactroot="">

---

</div><div class="api-docs__import" data-reactroot="">

```ts
import { RealtimeDBQueryParams } from "@hyper-fetch/firebase"
```

</div><div class="api-docs__section">

## Description

</div><div class="api-docs__description"><span class="api-docs__do-not-parse">



</span></div><p class="api-docs__definition">

Defined in [adapter/types/adapter.realtimedb.types.ts:63](https://github.com/BetterTyped/hyper-fetch/blob/c3e2c45/packages/adapter-firebase/src/adapter/types/adapter.realtimedb.types.ts#L63)

</p><div class="api-docs__section">

## Preview

</div><div class="api-docs__preview type">

```ts
type RealtimeDBQueryParams = {
  constraints: { toString: () => string; type: FirebaseQueryConstraints; values: any[] }[]; 
}
```

</div><div class="api-docs__section">

## Structure

</div><div class="api-docs__returns">

```ts
{
  constraints: {
    toString: () => string;
    type: FirebaseQueryConstraints;
    values: any[];
  }[];
}
```

</div>