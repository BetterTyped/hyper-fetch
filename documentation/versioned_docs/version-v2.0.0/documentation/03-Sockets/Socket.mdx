...
There are few methods for intercepting a response from a command:
...
Sockets has one generic type responsible for the type of the current `Socket Client`. It defaults to the built-in
adapter type.

```tsx
class Sockets<ClientType>
```

It needs to fulfill the `WebsocketClientType` interface.

(@import Sockets WebsocketClientType type=returns)
