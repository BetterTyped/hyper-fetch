...
[`useEmitter`](/documentation/04-React/03-Sockets/useEmitter.mdx) hook.
...
**`useEventMessages`** hooks into Socket client and start listening to given event when a component is mounted. It uses
...
(@import React UseEventMessagesOptionsType type=returns)
...
(@import React useEventMessages type=returns)
