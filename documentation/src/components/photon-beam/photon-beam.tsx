...
export function PhotonBeam(props: PhotonBeamProps = {}) {
  ...
  useEffect(() => {
    ...
    const c = new THREE.Color(props.colorBg ?? "#080808");
    sceneRef.current.background = c;
    if (sceneRef.current.fog instanceof THREE.FogExp2) {
      sceneRef.current.fog.color.copy(c);
    }
  }, [props.colorBg]);
  ...
  useEffect(() => {
    ...
    const init = (): void => {
      ...
      const params: Params = {
        ...
        globalRotation: props.globalRotation ?? 0,
        positionX: props.positionX ?? ((props.curveLength ?? 50) - (props.straightLength ?? 100)) / 2,
      ...
      camera.position.set(0, 0, props.cameraZ ?? 90);
      camera.lookAt(0, 0, 0);
      ...
      if (props.fitToViewport) {
        ...
        params.positionX = -visibleHalfWidth * (props.sourceXRatio ?? 0.72);
        params.straightLength = visibleHalfWidth * (props.endXRatio ?? 1.08) - params.positionX;
        params.curveLength = Math.max(params.curveLength, visibleHalfWidth * (props.curveXRatio ?? 0.4));
      }
      ...
      contentGroup.rotation.z = params.globalRotation;
      ...
      handleResize = (): void => {
        ...
        if (props.fitToViewport) {
          const visibleHalfH = Math.tan(THREE.MathUtils.degToRad(camera.fov / 2)) * camera.position.z;
          const visibleHalfW = visibleHalfH * camera.aspect;
          params.positionX = -visibleHalfW * (props.sourceXRatio ?? 0.72);
          params.straightLength = visibleHalfW * (props.endXRatio ?? 1.08) - params.positionX;
          params.curveLength = Math.max(props.curveLength ?? 50, visibleHalfW * (props.curveXRatio ?? 0.4));
          contentGroup.position.set(params.positionX, params.positionY, 0);
        }
      };
  ...
  return <div ref={containerRef} className={`h-full min-h-[200px] w-full bg-black ${props.className ?? ""}`} />;
}
