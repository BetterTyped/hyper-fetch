...
interface HexagonPatternProps extends React.SVGProps<SVGSVGElement> {
   ...
   * (filled) on top of the repeating pattern.
   */
...
function hexVertexList(cx: number, cy: number, r: number, direction: "horizontal" | "vertical"): HexPoint[] {
      ...
      {hexagons && hexagons.length > 0 && (
        <svg aria-hidden="true" className="overflow-visible" x={x} y={y}>
          {hexagons.map(([col, row]) => {
            ...
            return <polygon key={`${col}-${row}`} points={hexPoints(cx, cy, radius - 1, direction)} strokeWidth="0" />;
          })}
        </svg>
      )}
    </svg>
