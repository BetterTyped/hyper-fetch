...
export const Terminal = ({ children, className, sequence = true, startOnView = true }: TerminalProps) => {
  ...
  const content = (
    <div className={cn("bg-zinc-700/80 rounded-xl", className)}>
      <div
        ref={containerRef}
        className={cn(
          "relative z-0 h-full w-full rounded-xl bg-with-noise overflow-hidden",
          "border border-white/[0.08] [box-shadow:0_-20px_80px_-20px_#ffffff10_inset]",
        )}
      >
        <div className="flex flex-col gap-y-2 border-b border-white/[0.08] p-4 bg-zinc-900/50">
        ...
        <pre style={{ padding: "1rem", background: "transparent", margin: 0, fontSize: "inherit" }}>
          <code
