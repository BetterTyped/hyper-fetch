...
This hook controls the **dispatcher queues**. It uses the [`Dispatcher`](/docs/Core/Dispatcher) to read the actual value
of the queue, which is based on the `queueKey` retrieved from the command passed as the first argument.

A prepared [`Command`](/docs/Core/Command) is the minimum requirement for `useQueue`.
...
Read more about differences between `stop`, `pause` and `start` in the [`Dispatcher`](/docs/Core/Dispatcher) doc page.
