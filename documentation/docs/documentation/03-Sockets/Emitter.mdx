...
export const sendChatMessage = socketInstance.createEmitter<ChatMessageType>()({
  name: "chat-message", // name of the event
});

export const sendUserStatus = socketInstance.createEmitter<UserStatusType>()({
...
Make sure your server is supporting the acknowledgements!

:::

```tsx
sendChatMessage.emit({ data: { messsage: "some message" } }, () => {
