...
- `error` defines the global/default error type used in all the requests. It should consist of an `Error` type and your
  default `ServerErrorType`. It is propagated down to the `createRequest` method, serving as a default for the `globalError` type.

- `adapter` is the generic responsible for shaping options passed to the adapter. Most likely you will change it
  only when you provide [your custom adapter](/guides/02-advanced/custom-adapter.mdx).
