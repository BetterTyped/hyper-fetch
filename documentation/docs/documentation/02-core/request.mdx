...
const { data, error } = getUsers.send({ queryParams: { page: 1 } });

const { data, error } = getUsers.send({ params: { userId: 23 } });

const { data, error } = postUser.send({ data: { name: "Maciej" } });

const { data, error } = patchUser.send({ params: { userId: 12 }, data: { name: "Kacper" } });

const { data, error } = deleteUser.send({ params: { userId: 88 } });
```
...
const { data, error } = getCategory.setParams({ categoryId: 2 }).setQueryParams({ sortNotes: "age" }).send();
```
...
const { data, error } = request.send(); // Server error - no params
```
...
const { data, error } = requestWithParams.send(); // Success
```
