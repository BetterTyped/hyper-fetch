...
transmits data and takes care of validation in the system. Data is stored under `cacheKey` in the storage which by
...
We can achieve the persistence of the stored data by changing the cache storage to persistent one. It needs to match the
provided interface.
