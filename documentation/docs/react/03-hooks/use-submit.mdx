...
[`Submit Dispatcher`](/core/dispatcher.mdx) to handle requests and the [`Cache`](/core/cache.mdx) to manage the overall
state of the data.

The minimum requirement for `useSubmit` is a prepared [`Request`](/core/request.mdx).

If you intend to `retrieve` data from the server, we recommend choosing the [`useFetch`](/react/03-hooks/use-fetch.mdx)
hook.
...
Data and parameters can be passed in several ways. One option is to use methods on the [`Request`](/core/request.mdx),
such as `setData` or `setParams`.
