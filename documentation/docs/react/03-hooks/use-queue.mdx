...
the actual value of the queue, which is based on the `queryKey` retrieved from the request passed as the first argument.
...
**`useQueue`** uses the request to read the Dispatcher queue value based on its `queryKey`. Events emitted by the
...
Remember to make sure your request `queryKey` is static when you use this hook. Because it auto-generates keys, it may
return wrong values if your request uses params or query params. Providing a custom `queryKey` when the request is
