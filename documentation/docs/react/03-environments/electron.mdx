  ...
  appManager: () =>
    new AppManager({
      focusEvent: (setFocused) => {
        ...
        ipcRenderer.on("window-focused", focusListener);
        ipcRenderer.on("window-blurred", blurListener);

        return () => {
          ipcRenderer.removeListener("window-focused", focusListener);
          ipcRenderer.removeListener("window-blurred", blurListener);
        };
      },
    }),
});
