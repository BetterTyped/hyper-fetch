...
- **How to trigger mutations** with a simple `submit` function.
- **How to manage state** with automatic tracking of `submitting`, `error`, and `response`.
- **How to handle side-effects** using `onSubmitSuccess`, `onSubmitError`, and `onSubmitFinished`.
- **How to control requests** by dynamically passing data and parameters.
- **How to maintain data consistency** through automatic cache integration.
