...
**`FetchQueue`** is one of the hyper fetch queues. This is the place that handles the dispatch of requests and manages
their retries, offline handling, canceling and deduplication, up to sending the received data to the cache. This queue
is designed mainly for requests that are supposed to receive some data without mutating.
