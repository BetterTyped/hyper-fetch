...
`Hyper Fetch` was created with offline operation and data persistence in mind, thanks to this approach our queues, which
are responsible for sending and processing requests, are fully prepared for `creating offline-first applications`.
