...
chatMessageListener.listen(({ data }) => {
...
chatMessageEmitter.emit({ payload: { text: "Hello, world!" } });
```
...
<LinkCard
  type="docs"
  title="Emitters"
  description="How to send events to the server."
...
<LinkCard
  type="docs"
  title="Socket SDK"
  description="Type-safe proxy-based SDK for organizing listeners and emitters."
  to="/docs/sockets/sdk"
/>
