...
from each other - hooks from the fetchers. This of course gives the possibility to make it fully our own way - but there
are few issues coming with this appraoch. We need to build this logic every time we start building new application,
there is no schema for data exchange and normalization which is blocking many, many possible features to come. We
decided to overcome this issue by giving straight and opinionated solution with the flexibility that will make it
