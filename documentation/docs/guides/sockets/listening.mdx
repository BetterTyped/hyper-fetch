...
4.  How to integrate socket listeners in React using the **`useListener` hook**.
...
## React Integration with `useListener`

For React applications, `@hyper-fetch/react` provides a `useListener` hook that simplifies the process of listening to
socket events within your components. It automatically handles starting and stopping the listener based on the
component's lifecycle.

The `useListener` hook takes the listener instance as its first argument and returns an `onEvent` callback registrar
...
function ChatComponent() {
  ...
  // highlight-start
  // The useListener hook manages the listener's lifecycle,
  // automatically calling listen() on mount and unlisten() on unmount.
  // highlight-end
  const { onEvent } = useListener(onChatMessage);

  onEvent(({ data: newMessage }) => {
...
- You can integrate socket listeners into your **React components** effortlessly with the `useListener` hook.
