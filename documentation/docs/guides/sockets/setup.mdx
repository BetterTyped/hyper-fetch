...
For larger applications, consider using the [Socket SDK](/docs/sockets/sdk) instead of creating listeners and emitters
individually. It provides a tRPC-like, proxy-based interface where you define your schema once and access listeners and
emitters via dot-notation:
...
// Access listeners and emitters directly
sdk.chat.messages.$listener.listen(({ data }) => console.log(data));
sdk.chat.messages.$emitter.emit({ payload: { text: "Hello!" } });
```

The Socket SDK also supports centralized configuration with `$configure`, topic group matching, and plain-object
shorthand — see the [Socket SDK docs](/docs/sockets/sdk) for details.
