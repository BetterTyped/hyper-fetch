...
The most common case is forms which, apart from basic errors from the server, can return specific field errors of a
...
Here is how to set up a local error on a request. Notice how the `error` generic is used on the `createRequest`
...
// 5. Usage Example
const { data, error } = await createUser.send({ payload: { name: "John", email: "invalid-email" } });
