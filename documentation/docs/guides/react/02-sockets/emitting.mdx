...
4.  How to use helper hooks like **`onEmit`**, **`onError`**, and **`onReconnecting`** to manage the emitter's
    lifecycle.
...
function ChatInput() {
  ...
  const { emit, connected, onEmit, onError, onReconnecting } = useEmitter(sendMessage);
  ...
  const handleSubmit = (e) => {
    ...
    // Emit the event with payload as an options object
    // highlight-next-line
    emit({ payload });
    setMessage("");
  };
...
[sockets guide](/docs/guides/sockets/emitting). The `emit` function takes an options object with a `payload` property.
...
- You can monitor the emitter's lifecycle with **`onEmit`**, **`onError`**, and **`onReconnecting`**.
