...
2.  **Creating**: The `handleAddTodo` function calls `addTodo` with the new to-do's data. Because `submit` is a promise,
    we can `await` it and then run code after it completes, such as clearing the input field and calling `refetch()`
...
3.  **Updating**: `handleToggleTodo` calls `updateTodo` with the `todoId` in `params` and the new `completed` status in
    `payload`. We then refetch to show the change.
4.  **Deleting**: `handleDeleteTodo` calls `deleteTodo` with the appropriate `todoId` and refetches.
...
- You can **refetch** data from `useFetch` to keep your UI in sync with server changes.
