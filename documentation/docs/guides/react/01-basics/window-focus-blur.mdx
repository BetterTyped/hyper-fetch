...
3.  The difference between **`refetchOnFocus`**, **`refetchOnBlur`**, and **`refetchOnReconnect`**.
4.  How `refetchBlurred` affects background refreshing behavior.
...
function WindowEventsDemo() {
  const [options, setOptions] = React.useState({
    refresh: true,
    refreshTime: Time.SEC * 5,
    refetchOnFocus: true,
    refetchOnBlur: false,
    refetchOnReconnect: true,
    refetchBlurred: false,
  });
  ...
  const { data, onSuccess } = useFetch(getActivities, options);
  ...
  return (
    <div className="border rounded-md">
      <div className="p-4 border-b">
        ...
        <OptionToggle name="refetchOnFocus" description="Refetch when you return to this tab." />
        <OptionToggle name="refetchOnReconnect" description="Refetch when network connection is restored." />
        <OptionToggle name="refetchOnBlur" description="Refetch when you leave this tab." />
        <OptionToggle
          name="refetchBlurred"
...
- **`refetchOnFocus`**: (Default: `false`) When set to `true`, data will be
...
- **`refetchOnReconnect`**: (Default: `false`) If the user loses their network connection and later comes back online,
  this option will automatically trigger a re-fetch. This helps your app gracefully recover from network interruptions.

- **`refetchOnBlur`**: (Default: `false`) When `true`, a re-fetch is triggered the moment the user leaves the window
...
- **`refetchBlurred`**: (Default: `true`) This option works in conjunction with interval-based refreshing (using
  `refresh` and `refreshTime`). When `true`, interval-based refreshing will continue to run even when the tab is in the
  background. Set to `false` to pause interval refreshing when the window is blurred (not focused) and save resources.
...
- You understand the specific behaviors of **`refetchOnFocus`**, **`refetchOnBlur`**, and **`refetchOnReconnect`**.
- You can control **background data polling** with the `refetchBlurred` option.
