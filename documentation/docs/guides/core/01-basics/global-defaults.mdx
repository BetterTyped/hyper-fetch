...
// highlight-next-line
console.log(getUsers.cacheKey); // Cache__GET__/users
```
...
// The custom ID is assigned when the request enters the dispatcher
const requestId = client.fetchDispatcher.add(getUsers);
console.log(requestId); // Request__/users__1718858400000
```
...
## SDK Configuration Alternative

If you're using the [SDK pattern](/docs/core/sdk), the `$configure` method provides an even more powerful way to set
defaults. It supports **method-specific keys** (like `"users.$get"`), **endpoint groups** (like `"/users/*"`), and
**function-based values** that give you access to every Request setter — including mappers, mocking, and hooks.

```typescript
const configured = sdk.$configure({
  "*": { retry: 3, deduplicate: true },
  "/users/*": { cache: true, cacheTime: 60000 },
  "users.$get": (request) => request.setResponseMapper(userListMapper),
  "users.$post": { retry: 0, cache: false },
});
```

See the [SDK documentation](/docs/core/sdk#sdk-configuration-configure) for full details.
