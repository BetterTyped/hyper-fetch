...
Later, when a component needs this data (for instance, using `useFetch` in React), Hyper Fetch will find it in the cache
...
When you later try to fetch the same data (for example, with `useFetch` in a React component), Hyper Fetch checks the
cache first. If a fresh (not stale) response is found, it's returned immediately, and **no network request is made**.
This is what creates the "instant" experience for the user.
