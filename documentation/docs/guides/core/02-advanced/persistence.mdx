...
const asyncStorage: CacheAsyncStorageType = {
  set: (key, data) => set(idbValidKey, adapter),
...
we will have multiple clients initialized where the **duplicated** requests will be send and it may cause major db data
damage.
