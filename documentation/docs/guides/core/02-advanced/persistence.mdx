...
You can make `useFetch` requests persistent by providing storage to the `fetchDispatcher`.
...
Similarly, `useSubmit` requests can be persisted by providing storage to the `submitDispatcher`.
