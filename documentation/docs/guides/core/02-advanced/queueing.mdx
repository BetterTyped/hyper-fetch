...
function Example() {
  ...
  const updateStatus = (message) => {
    setStatuses((prev) => [`${new Date().toLocaleTimeString()}: ${message}`, ...prev]);
  };

  const fileRequest = client.createRequest()({
    endpoint: "/shared-port/1/files",
    method: "POST",
    queued: true,
  });
  ...
  const handleSend = () => {
    ...
    updateStatus(`Sending 4 requests...`);
    requests.forEach((req) => {
      fileRequest.send({
        onStart: () => updateStatus(`Request ${req} started.`),
        onFinished: ({ isSuccess }) =>
          updateStatus(`Request ${req} finished ${isSuccess ? "successfully" : "with an error"}.`),
      });
    });
  };
