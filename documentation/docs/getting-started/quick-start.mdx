...
The first step in implementing Hyper Fetch is to initialize the **[Client](/core/client.mdx)**. It manages the basic
configuration for connection to the server and all Hyper Fetch’s essential elements (i.e. instances of **dispatchers**,
**cache**, and **app managers**). Start by determining the `url` of your server.
...
const postLogin = client.createRequest<{ response: ResponseType; request: RequestType }>()({
  method: "POST",
  endpoint: "/auth/login",
});
```
