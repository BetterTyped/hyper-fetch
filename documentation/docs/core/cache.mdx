...
Hyper Fetch automatically detects the runtime environment and configures cache behavior accordingly:

- **Client mode** (browser): Cache works for all requests by default. `setScope` is optional and acts as a key prefix
  for organizing data (e.g. per feature, per workspace).
- **Server mode** (Node.js): Cache is **disabled by default**. A request is only cached if `.setScope()` is explicitly
  set. This prevents cross-user data leaks in server environments.
