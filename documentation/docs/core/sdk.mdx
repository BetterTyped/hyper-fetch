...
2.  **Intuitive, tRPC-like Syntax**: Access your endpoints with a clean, dot-notation syntax (e.g., `sdk.users.get()`).
3.  **On-the-fly Request Generation**: Uses recursive proxies to automatically create `Request` instances with the
    correct method and endpoint as you access them.
4.  **Extremely Performant**: Because requests are generated on-demand, there's virtually no performance overhead, no
    matter how large your API is.
5.  **Dynamic Paths**: Easily handle dynamic URL segments like `/users/:id`.
6.  **Centralized Definition**: Keep your API structure defined in one place, making it easy to manage and update.
