...
you to define your entire API surface as a typed object, which then generates request instances for you. This approach
promotes a highly organized and maintainable codebase, where endpoints are accessed with intuitive dot-notation.

---

:::tip Purpose

1.  **Type-Safe API**: Define your entire API with TypeScript for end-to-end type safety.
2.  **Intuitive Syntax**: Access your endpoints with a clean, dot-notation syntax (e.g., `sdk.users.get()`).
3.  **Automatic Request Generation**: Automatically creates `Request` instances with the correct method and endpoint.
4.  **Dynamic Paths**: Easily handle dynamic URL segments like `/users/:id`.
5.  **Centralized Definition**: Keep your API structure defined in one place, making it easy to manage and update.
