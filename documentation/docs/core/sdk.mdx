...
:::tip Use `RequestModel` for schema leaves — not `RequestInstance`

When defining a schema you are describing **one specific endpoint**. Use `RequestModel<{...}>` — omitted fields stay
...
:::info Client injection — you do not pass `client` per request

You do not have to add `client: AppClient` to every `RequestModel` declaration. `createSdk(client)` rewrites the schema
...
<LinkCard
  type="docs"
  title="Extend Request — the `RequestInstance` constraint pattern"
  description="When (and how) to use RequestInstance to build reusable components that accept any matching request."
  to="/docs/guides/typescript/extend"
/>
...
2.  `.$get`: This access is caught by the new proxy. It recognizes the `$`-prefixed key as an HTTP method, finalizes
    the request configuration (`GET /users`), and returns a fully configured `Request` instance, ready to be used.
