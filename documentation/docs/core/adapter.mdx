...
response. This is used internally by the [`useStream`](/docs/react/hooks/use-stream) hook.

```ts title="Streaming a response"
...
:::tip React Integration

For React apps, use the [`useStream`](/docs/react/hooks/use-stream) hook instead of manually consuming the stream.
