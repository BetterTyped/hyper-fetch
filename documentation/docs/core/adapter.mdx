...
> The `Adapter` class is the backbone of Hyper Fetch's communication system. It abstracts the details of how requests
> are executed, allowing you to swap between HTTP, GraphQL, Firebase, or any custom transport layer with ease. Adapters
> are responsible for executing requests, mapping payloads and headers, handling query parameters, and integrating with
> the request lifecycle—including progress tracking, cancellation, and error handling.
