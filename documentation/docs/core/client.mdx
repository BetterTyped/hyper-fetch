...
Client has three generic types that can be passed when initializing a client.

```tsx
import { ClientErrorType, Client, AdapterType, DefaultEndpointMapper } from "@hyper-fetch/core";

const client = new Client<{ error: ClientErrorType; adapter: AdapterType; mapper: DefaultEndpointMapper }>();
```
...
- `adapter` is the generic responsible for shaping options passed to the adapter. Most likely you will change it only
  when you provide [your custom adapter](/guides/02-advanced/custom-adapter.mdx).
