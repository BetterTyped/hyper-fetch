...
5. ### Cache Modes (Client / Server / Isomorphic)

The `Client` automatically detects whether it's running in the browser or on the server and sets its mode accordingly.
This affects how caching behaves — on the server, caching is **disabled by default** to prevent cross-user data leaks.

```tsx
import { createClient } from "@hyper-fetch/core";

const client = createClient({ url: "http://localhost:3000" });
// Mode is auto-detected: "client" in browser, "server" in Node.js
```
...
<LinkCard
  type="docs"
  title="Cache"
  description="Learn more about cache modes, scoping, and persistence."
  to="/docs/core/cache"
/>
