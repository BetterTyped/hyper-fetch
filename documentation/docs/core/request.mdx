...
The `send()` method accepts a `cachePolicy` option that controls how the request interacts with the cache:

- **`"network-only"`** (default) - Always fetches from the network, ignoring any cached data.
- **`"cache-first"`** - Returns cached data if available and not stale. Falls back to a network request otherwise.
- **`"revalidate"`** - Returns cached data immediately (if available), then fetches from the network in the background
  to refresh the cache (stale-while-revalidate pattern).

```ts
// Default - always goes to network
const { data } = await getUser.send();

// Use cache if available, otherwise fetch
// highlight-next-line
const { data } = await getUser.send({ cachePolicy: "cache-first" });

// Return cache immediately, refresh in background
// highlight-next-line
const { data } = await getUser.send({ cachePolicy: "revalidate" });
```

##### Dispatcher Type

You can force which dispatcher handles a request using the `dispatcherType` option:

- **`"auto"`** (default) - GET requests go to `fetchDispatcher`, mutations go to `submitDispatcher`.
- **`"fetch"`** - Force use of `fetchDispatcher` regardless of method.
- **`"submit"`** - Force use of `submitDispatcher` regardless of method.

```ts
// Force a GET request through the submit dispatcher
const { data } = await getUser.send({ dispatcherType: "submit" });
```
...
When creating a request, the following defaults are applied unless overridden:

| Option | Default | Description |
| --- | --- | --- |
| `auth` | `true` | Whether auth interceptor runs for this request |
| `method` | Adapter's default method | HTTP method (typically `"GET"`) |
| `cancelable` | `false` | Whether sending a new request auto-cancels the previous one |
| `retry` | `0` | Number of retry attempts on failure |
| `retryTime` | `500` (ms) | Delay between retries |
| `cacheTime` | `300000` (5 min) | How long cached data is stored |
| `cache` | `true` | Whether the response is cached |
| `staleTime` | `300000` (5 min) | How long cached data is considered fresh |
| `queued` | `false` | Whether requests are queued (sequential execution) |
| `offline` | `true` | Whether requests are stored when offline |
| `deduplicate` | `false` | Whether identical in-flight requests are deduplicated |
| `deduplicateTime` | `null` | Custom deduplication window (uses request duration when `null`) |

---

### Disabling Interceptors

Individual requests can opt out of client-level interceptors. This is useful when a specific endpoint should bypass
global auth or response processing logic.

```ts
const publicRequest = client.createRequest()({
...
When `disableRequestInterceptors` is `true`, the `onRequest` and `onAuth` interceptors are skipped. When
`disableResponseInterceptors` is `true`, the `onResponse`, `onSuccess`, and `onError` interceptors are skipped.
...
Use `setRetryOnError` to decide at runtime whether a failed request should be retried. The callback receives the error
response and returns a boolean. When it returns `false`, retries are skipped even if the `retry` count has not been
exhausted.

```ts
const request = client
  .createRequest()({ endpoint: "/unstable", retry: 3, retryTime: 1000 })
  // highlight-start
  .setRetryOnError((response) => {
    // Only retry on server errors, not on 4xx client errors
...
Use `setScope` to isolate all keys (cache, queue, abort) by scope (user, session, tenant, etc.). On the server, this
is required to enable caching at all. On the client, it acts as an optional key prefix for organization.
...
// Scoped cache — isolated per session
const scopedRequest = getUser
  .setParams({ userId: 1 })
  .setScope(sessionId);

const { data } = await scopedRequest.send();
```

:::info

`setScope` returns a new cloned request (immutable). The scope string is included in `toJSON()` so it survives
serialization through dispatchers and queues. All keys — cache, queue, and abort — are prefixed with the scope,
ensuring complete isolation between scoped requests.
...
Optimistic updates let you update the UI immediately before the server responds, then automatically rollback on failure.
Configure optimistic behavior directly on the request with `setOptimistic`:
...
The callback receives `request`, `client`, and `payload`. It returns:

- **`context`** — arbitrary data available as `mutationContext` in all lifecycle callbacks (`onResponse`, `$hooks`, React
  hooks). The type is auto-inferred.
- **`rollback`** — called automatically on failure or abort to revert cache changes.
- **`invalidate`** — list of requests whose cache is refreshed on success.

All fields are optional. The callback runs before the request is dispatched — if it throws, the request is not sent
and an error response is returned.

`setOptimistic` is type-safe: `mutationContext` is available in `send()` callbacks, `$hooks` listeners, and React hooks
(`onSubmitSuccess`, `onSubmitError`, etc.) with its type automatically inferred from the `context` return value.

```typescript
// send() callback — mutationContext is typed
patchUser.send({
  params: { userId: 1 },
  payload: { name: "New Name" },
  onResponse: ({ mutationContext }) => {
    console.log(mutationContext?.snapshot);
  },
});

// $hooks — mutationContext flows through
patchUser.$hooks.onResponse(({ mutationContext }) => {
  console.log("Snapshot:", mutationContext?.snapshot);
});
```
...
<LinkCard
  ...
  description="Learn how to implement optimistic updates with rollback, invalidation, and concurrent mutations."
...
The `$hooks` API provides **persistent, instance-level lifecycle hooks** on a `Request`. Unlike per-`send()` callbacks
(which are one-shot), `$hooks` callbacks run on every send of that request instance. They survive `.clone()` and are
ideal for cross-cutting concerns like logging, analytics, or auth token injection.

Each hook method registers a callback and returns an **unsubscribe function**. Multiple listeners per hook are supported.

```tsx title="Adding persistent hooks"
const getUser = client.createRequest<{ response: User }>()({
  endpoint: "/users/:userId",
  method: "GET",
});

getUser.$hooks.onBeforeSent(({ requestId }) => {
  console.log(`Request ${requestId} about to be sent`);
});

const unsubscribe = getUser.$hooks.onResponse(({ response }) => {
  analytics.track("api_call", { endpoint: "/users/:userId", status: response.status });
});

// Hooks fire on every send
...
`$hooks` is a **runtime-only** property. It is excluded from `toJSON()` serialization, so hooks are not persisted
to storage or queues. When a request is restored from JSON, hooks are simply not present (no crash).

The `$` prefix convention indicates runtime-only properties throughout Hyper Fetch.
