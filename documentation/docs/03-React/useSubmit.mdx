...
The minimum requirement to use `useSubmit` is to have a prepared [`Command`](/docs/Architecture/Command).

If you intend to `retrieve` data from the server, we recommend [`useFetch`](/docs/React/useFetch) hook.
...
tracking to limit re-rendering and help with the performance. Under the hood the communication with the core systems is
...
readability of the code and increase complication level. Thanks to this we can easily organize our code.
...
[`Command`](/docs/Architecture/Command) such as `setData` or `setParams`.
