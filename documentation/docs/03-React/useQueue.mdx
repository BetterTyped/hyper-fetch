...
This hook is created to **control the dispatcher queues**. It uses the [`Dispatcher`](/docs/Architecture/Dispatcher) to
read the actual value of the queue `based on the queueKey` retrieved from command passed as first argument.

The minimum requirement to use `useQueue` is to have a prepared [`Command`](/docs/Architecture/Command).
...
Remember to make sure your command `queueKey` is static when you use this hook, because of the auto-generation of keys it
...
Read more about differences between `stop`, `pause` and `start` at the [`Dispatcher`](/docs/Architecture/Dispatcher) doc
page.
