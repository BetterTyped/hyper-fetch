...
export const builder = async (context: LoadContext, options: PluginOptions) => {
  const { generatedFilesDir } = context;
  const { apiDir = "", packages } = options;
  const isMonorepo = packages.length > 1;

  info(`Generate docs for ${packages.length} package`);

  const docsRoot = path.join(generatedFilesDir, name);
