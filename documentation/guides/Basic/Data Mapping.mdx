...
One of them are `breaking changes to the server api`, which may require many changes, which may be dangerous for the
application and introduce **feature regression**.
...
in the command and then map everything to FormData just before it gets added to **[Dispatcher](/docs/Docs/Dispatcher)**.

In this way, we ensure very safe typing and flexibility in developing our application.
