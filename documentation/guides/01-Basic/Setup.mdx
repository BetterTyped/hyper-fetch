...
The first step is to initialize the **[Builder](/docs/Core/Builder)**. It manage the basic configuration for connection
to the server and all the elements that make up Hyper Fetch, which are instances of - **dispatchers**, **cache** and
**app managers**. We start by determining the `url` of our server.
